//
//  TISAlignCollectionViewFlowLayout.m
//  TisComponent
//
...
@interface TISAlignCollectionViewFlowLayout ()

@property (nonatomic) CGFloat sumWidth; // 在居中对齐的时候需要知道这行所有cell的宽度总和

@end

@implementation TISAlignCollectionViewFlowLayout
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.betweenOfCell = 10.0;
        self.alignType = AlignWithLeft;
    }
    return self;
}
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (NSUInteger index = 0; index < layoutAttributes.count ; index++) {
        ...
        self.sumWidth += currentAttr.frame.size.width;
...
- (void)setCellFrameWith:(NSMutableArray*)layoutAttributes {
    ...
    switch (self.alignType) {
        ...
        case AlignWithCenter:
            nowWidth = (self.collectionView.frame.size.width - self.sumWidth - (layoutAttributes.count * self.betweenOfCell)) / 2;
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                CGRect nowFrame = attributes.frame;
                nowFrame.origin.x = nowWidth;
                attributes.frame = nowFrame;
